...
personal_consumption.df <- tibble(
           ...
           state      = word_extractor(string = file_list, sep = "[_]", index = 2),
...
geo_states <- map_data("state") %>% mutate(region = ifelse(str_detect(region, "^[:upper:]+$"), region,str_to_title(region)))
...
percent_gg <- ggplot(geo_percent_change.df, aes(long, lat)) + 
 ...
 labs(
      x = "Longitude",
      y = "Latitude",
      title = "Total Consumption Change (%) for different State Compared with 2018"
    ) +
